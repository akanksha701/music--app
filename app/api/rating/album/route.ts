...
async function updateAverageRating(avgRatingResult: Array<{ _id: string, averageRating: number }>, albumId: string) {
  ...
  const updateResult = await db.collection('albums').findOneAndUpdate(
...
export async function POST(req: Request) {
  try {
    const { userId, albumId, rating } = await req.json();
    if (!userId || !albumId || typeof rating !== 'number' || rating < 1 || rating > 5) {
      return NextResponse.json(
        { message: 'Invalid input data. Please provide a valid userId, albumId, and a rating between 1 and 5.' },
        { status: 400 }
      );
    }
    ...
    await updateAverageRating(avgRatingResult as Array<{ _id: string, averageRating: number }>, albumId);
    return NextResponse.json(
