...
const AlbumPage = ({ params }: { params: Usable<{ id: string }> }) => {
  ...
  const { isLoading: isLoadingUser, isError: isErrorUser } =
  ...
  if (isError) {
    return (
      <div className="text-center text-xl">
        Album not found or an error occurred!
      </div>
    );
  }

  const album: Album = albumData.data;
  return (
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-gray-500">
                Genres: {album.Genre?.map((genre) => genre.name).join(', ')}
              </span>
              <span className="text-gray-500">
                Languages:{' '}
                {album.Language?.map((language) => language.name).join(', ')}
              </span>
            </div>
          </div>
