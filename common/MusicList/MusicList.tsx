  ...
  },
  {
    ...
    renderCell: (params) => (
      <div className="flex flex-col items-center gap-2 justify-center  ">
        <div style={{ width: "50px", height: "50px", borderRadius: "4px" }} >{params.value}</div> 
        <div className="text-xs text-gray-500 flex flex-col" style={{ width: "50px", height: "50px", borderRadius: "4px" }} >
          ...
          <span className="w-fit text-[0.6rem] flex gap-1 items-center">
            ...
            <p className="bg-purple-500 rounded-lg px-1 py-0.5 text-white"> {params.row.language}</p>
          </span>
        </div>
      </div>
    ),
      ...
      <div className="flex flex-col items-center gap-4 justify-center  ">
        ...
        <div className="text-xs text-gray-500 flex flex-col" style={{ width: "50px", height: "50px", borderRadius: "4px" }} >
          <span className="w-fit text-[0.6rem] flex gap-1 items-center">
            ...
            <p className="bg-purple-500 rounded-lg px-1 py-0.5 text-white"> {params.row.genre}</p>
          </span>
        </div>
      </div>
    ),
...
const MusicList = ({ data, MyselectedSongs, mode, prevData }: { data: Array<any>; MyselectedSongs: Array<string>; mode: string ; prevData: any }) => {
  const [selectedSongs, setSelectedSongs] = useState<any[]>(MyselectedSongs); // State to store selected rows
  const [filteredData, setFilteredData] = useState(data);
  const [selectionModel, setSelectionModel] = useState<GridRowSelectionModel>(MyselectedSongs); // Initialize row selection with MyselectedSongs
  ...
  const handleSelectionChange = (selectionModel: GridRowSelectionModel) => {
    setSelectionModel(selectionModel); // Update selection model
    const selectedRows = rows.filter((row) => selectionModel.includes(row.id)); // Get selected rows based on selectionModel
    setSelectedSongs(selectedRows); // Update selected songs state
  };
  ...
  useEffect(() => {
    // Map selected songs to rows only if rows are available
    if (MyselectedSongs.length > 0 && rows) {
      const initialSelectedRows = rows.filter((row) =>
        MyselectedSongs.includes(row.id)
      );
      setSelectionModel(MyselectedSongs); // Update selection model
      setSelectedSongs(initialSelectedRows); // Update selected songs
    }
  }, [MyselectedSongs]); // Only run when MyselectedSongs changes
  

  return (
    <div className="px-8 pb-8 flex flex-col">
      <div className="header-box flex justify-between align-center">
        <h1 className="text-2xl font-bold" style={{ width: "33%" }}>My Tracks</h1>
        ...
        >
          {selectedSongs.length >= 2 && (
            <PrimaryButton name={ mode === "edit" ? "Edit Album" : "Create Album"} mode={mode} onClick={handleCreateAlbum} />
          )}
